...
watch(() => item.value.interval_value, (newVal, oldVal) => {
  if ((!oldVal || oldVal == 1) && newVal > 1) intervals.value.forEach(i => i.title = `${i.title}s`)
...
v-card(width="350")
  ...
  v-card-text
    v-form(ref="form", @submit.prevent="submit")
      v-row
        v-col
          v-text-field.mb-1(placeholder="Name", v-model="item.name", prepend-icon="mdi-transcribe", :rules="[required]")
      v-row
      ...
      v-row
        v-col.d-flex.align-center.pt-0
          v-text-field(placeholder="Value", v-model="item.value", type="number", prepend-icon="mdi-cash-multiple", :rules="[required]")
      v-row
        v-col.pt-0
          v-text-field(:label="isRecurring ? 'Next Date' : ''", :placeholder="isRecurring ? '' : 'When'" v-model="startDate", read-only, @click:control="showStartDatePicker=true", prepend-icon="mdi-calendar-today", :rules="[required]", validate-on="lazy")
        ...
        v-col.py-0
          ...
          template.mt-1(v-if="isRecurring")
            .px-3
              v-row(dense)
                v-col(cols="3")
                  v-text-field.mt-1(label="Every", v-model="item.interval_value", type="number", :min="1", :rules="isRecurring ? [required] : null")
                v-col
                  v-select.mt-1(placeholder="Interval", v-model="item.interval", :items="intervals", :rules="isRecurring ? [required] : null")
              v-text-field(v-if="isRecurring", :label="`End${endDate ? 'ing' : ' Date?'}`", v-model="endDate", read-only, @click:control="showEndDatePicker=true", clearable, :class="endDate ? 'mt-2' : 'mt-1'")
